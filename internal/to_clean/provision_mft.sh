...
##set specific vars
TEMPLATE_PATH_DEFAULT="sag-mft/template.yaml"
TEMPLATE_PROPS_DEFAULT="mft"
...
do
    ...
    case "$KEY" in
            ...
            LICENSE_KEY_ALIAS) LICENSE_KEY_ALIAS=${VALUE} ;;
...
if [ "x$LICENSE_KEY_ALIAS" = "x" ]; then
    echo "error: Variable LICENSE_KEY_ALIAS (for MFT) is required...exiting!"
    exit 2;
fi
...
$SAGCCANT_CMD -Denv.CC_CLIENT=$CC_CLIENT \
              ...
              -Dmft.license.key.alias=$LICENSE_KEY_ALIAS \
              -Drepo.product=$REPO_PRODUCTS \
              -Drepo.fix=$REPO_FIXES \
              -Dmft.fixes=$FIXES \
              -Dmft.administrator.password=$DEFAULT_ADMIN_PASSWORD \
              -Ddb.type=${db_type} \
              -Ddb.host=${db_host} \
              -Ddb.port=${db_port} \
              -Ddb.sid=${db_sid} \
